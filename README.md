...
> 让 AI agent 拥有干净、按需加载、可溯源的项目知识。

## 一句话说明

**Lore 让开发者的隐性知识（WHY、约束、架构）随代码一起版本化、传播、积累，并按模块分区存储，让 AI agent 每次对话都有干净的上下文。**
...
Lore 将知识分为四层，各有不同的来源和管理方式：

| 层级 | 名称 | 来源 | 内容 | 更新方式 |
|------|------|------|------|---------|
| **L0** | 设计层 | 人工编写 | API 设计、数据模型、设计意图 | `/lore-spec` 手动创建 |
...
**模块化分区**：知识按代码模块分区存储，agent 按需加载，上下文始终保持干净。
...
| `/lore-evolve` | 编译 snapshot + 模块路由 | 自动/手动 |
| `/lore-health` | 知识健康校验 | 自动（SessionStart） |
| `/lore-sync` | L1 结构扫描 + 模块发现 | 手动 |
| `/lore-constraint` | L2 约束管理 | 手动 |
| `/lore-import` | 文档/重型知识导入 | 手动 |
| `/lore-modularize` | 自动模块发现与知识分区 | 自动（首次 evolve） |
...
| 知识校验 | ❌ | ❌ | ✅ /lore-health |
| **模块化分区** | ❌ | ❌ | ✅ 按代码模块分区 |
| **按需加载** | ❌ 全量加载 | ❌ 全量加载 | ✅ agent 按需读取 |
| **设计意图** | ❌ | ❌ | ✅ L0 prescriptive 层 |
